...
Realm settings → User profile → Create attribute
  Name: allowedTenantIds, Multivalued: yes, Admin-only edit

Users → <user> → Details → Allowed Tenant IDs
  enter: tenant-a, tenant-b, tenant-c

Clients → <client> → Client scopes → <client>-dedicated → Mappers
  → Add mapper → By configuration → User Attribute
  User Attribute:        allowedTenantIds
  Token Claim Name:      allowedTenantIds
  Claim JSON Type:       String
  Multivalued:           ON        ← the important one
  Add to ID token:       ON
...
That's the full path. The `Multivalued` toggle on the mapper is the one switch that turns it from a single string into the `["tenant-a","tenant-b","tenant-c"]` array you want.
